...
double curDistFlownFor(double, double);
void printPumpkin(double, double, double, double);
...
int main() {
    ...
    while(pumpkinCurrentAltitude_m > GROUND_ALTITUDE_M) {
        ...
        double distanceFlownForward = curDistFlownFor(pumpkinCurrentForwardVelocity_mps, secondsSimulated_s);

        
        pumpkinCurrentForwardDistance_m = pumpkinCurrentForwardDistance_m + distanceFlownForward;
 
        printPumpkin(pumpkinCurrentAltitude_m, pumpkinCurrentForwardDistance_m, currentTime_ms, pumpkinCurrentFallingVelocity_ms);
    }
...
double curDistFlownFor(double curForVel_mps, double secondsSim) {
    double result = curForVel_mps;
    result = curForVel_mps * secondsSim;
    return result;
}
void printPumpkin(double pumCuralt_m, double curForFlown_m, double time, double pumCurFallVel) {
